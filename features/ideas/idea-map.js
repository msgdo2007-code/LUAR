(() => {
  ...
  function syncFilterToggle() { const button = root?.querySelector("#ideaMapFilterToggle"); if (!button) return; const count = [categorySelect?.value, prioritySelect?.value, statusSelect?.value].filter(value => value && value !== "all").length, badge = button.querySelector("b"); badge.textContent = count; button.classList.toggle("has-filters", count > 0); button.setAttribute("aria-label", count ? `Filtros, ${count} ativos` : "Abrir filtros"); }
  function populateCategories() { if (!categorySelect) return; const legacy = [...new Set(notes().filter(note => !note.categoryId && note.category).map(note => note.category))]; categorySelect.replaceChildren(new Option("Todas as categorias", "all"), new Option("Sem categoria", "none"), ...categories.map(item => new Option(`${item.icon} ${item.name}`, item.id)), ...legacy.map(name => new Option(`${name} · antiga`, `legacy:${name.toLocaleLowerCase("pt-BR")}`))); categorySelect.value = [...categorySelect.options].some(option => option.value === mapState().selectedCategory) ? mapState().selectedCategory : "all"; syncFilterToggle(); }

  function build() {
    root = document.getElementById("ideaMapApp"); if (!root || root.dataset.ready) return; root.dataset.ready = "true";
    root.innerHTML = `<article class="card idea-map-shell"><div class="idea-map-toolbar"><div class="idea-toolbar-main"><div class="idea-view-tabs"><button class="active" type="button" data-idea-view="map">✦ Mapa</button><button type="button" data-idea-view="list">☷ Lista</button></div><label class="idea-theme-compact" title="Formato do mapa"><i>◍</i><select id="ideaMapTheme" aria-label="Formato do mapa">${Object.entries(THEMES).map(([value, text]) => `<option value="${value}">${text}</option>`).join("")}</select></label><label class="idea-search-compact"><i>⌕</i><input id="ideaMapSearch" type="search" placeholder="Buscar uma ideia..."></label><button type="button" id="ideaMapFilterToggle" class="idea-filter-toggle" aria-expanded="false" aria-controls="ideaMapFilters"><i>≡</i><span>Filtros</span><b>0</b></button><div class="idea-map-actions"><div class="idea-zoom-controls" aria-label="Controles de zoom"><button type="button" id="ideaMapZoomOut" aria-label="Diminuir zoom" title="Diminuir zoom">−</button><button type="button" id="ideaMapZoomIn" aria-label="Aumentar zoom" title="Aumentar zoom">＋</button></div><button type="button" id="ideaMapCenter" aria-label="Centralizar mapa" title="Centralizar mapa">◎</button><button type="button" id="ideaMapArrange" class="idea-arrange-button" aria-label="Reorganizar estrelas" title="Reorganizar estrelas">✦</button></div></div><div class="idea-filter-drawer" id="ideaMapFilters" hidden><label><small>Categoria em destaque</small><select id="ideaMapCategory" aria-label="Categoria destacada"></select></label><label><small>Prioridade</small><select id="ideaMapPriority" aria-label="Filtrar por prioridade"><option value="all">Todas</option><option value="high">Alta</option><option value="medium">Média</option><option value="low">Baixa</option></select></label><label><small>Status</small><select id="ideaMapStatus" aria-label="Filtrar por status"><option value="all">Todos</option><option value="active">Ativa</option><option value="paused">Pausada</option><option value="completed">Concluída</option></select></label></div></div><div class="idea-map-layout"><div class="idea-canvas-wrap"><canvas id="ideaMapCanvas" aria-label="Mapa visual de ideias"></canvas><div class="idea-map-help">Arraste estrelas · Arraste o fundo · Use zoom para explorar</div></div><aside class="idea-detail" id="ideaMapDetail"></aside></div></article>`;
    ...
    categorySelect.onchange = () => { mapState().selectedCategory = categorySelect.value; camera = { x: 0, y: 0, zoom: 1 }; syncFilterToggle(); layout(); persist(); };
    const filterToggle = root.querySelector("#ideaMapFilterToggle"), filterDrawer = root.querySelector("#ideaMapFilters"); filterToggle.onclick = () => { const opening = filterDrawer.hidden; filterDrawer.hidden = !opening; filterToggle.setAttribute("aria-expanded", String(opening)); };
    searchInput.oninput = draw; prioritySelect.onchange = () => { syncFilterToggle(); draw(); }; statusSelect.onchange = () => { syncFilterToggle(); draw(); }; root.querySelector("#ideaMapZoomOut").onclick = () => { camera.zoom = Math.max(.35, camera.zoom * .82); draw(); }; root.querySelector("#ideaMapZoomIn").onclick = () => { camera.zoom = Math.min(2.8, camera.zoom * 1.22); draw(); }; root.querySelector("#ideaMapCenter").onclick = () => { camera = { x: 0, y: 0, zoom: 1 }; draw(); }; root.querySelector("#ideaMapArrange").onclick = () => { Object.values(mapState().positions[mapState().theme]).forEach(point => point.locked = false); layout(); persist(); };
