(() => {
  ...
  function build() {
    ...
    root.innerHTML = `<article class="card idea-map-shell"><div class="idea-map-toolbar"><div class="idea-toolbar-primary"><div class="idea-view-tabs"><button class="active" type="button" data-idea-view="map">✦ Mapa</button><button type="button" data-idea-view="list">☷ Lista</button></div><label class="idea-tool-field idea-theme-field"><small>FORMA DO MAPA</small><select id="ideaMapTheme" aria-label="Tema do mapa">${Object.entries(THEMES).map(([value, text]) => `<option value="${value}">${value === "free" ? "⌘" : value === "blackhole" ? "◉" : value === "earth" ? "◍" : "◐"} ${text}</option>`).join("")}</select></label><label class="idea-search-field"><i>⌕</i><span><small>BUSCAR NO MAPA</small><input id="ideaMapSearch" type="search" placeholder="Pesquisar ideias"></span></label></div><div class="idea-toolbar-secondary"><div class="idea-filter-group"><span>FILTROS</span><label class="idea-tool-field"><small>Categoria em destaque</small><select id="ideaMapCategory" aria-label="Categoria destacada"></select></label><label class="idea-tool-field"><small>Prioridade</small><select id="ideaMapPriority" aria-label="Filtrar por prioridade"><option value="all">Todas</option><option value="high">Alta</option><option value="medium">Média</option><option value="low">Baixa</option></select></label><label class="idea-tool-field"><small>Status</small><select id="ideaMapStatus" aria-label="Filtrar por status"><option value="all">Todos</option><option value="active">Ativa</option><option value="paused">Pausada</option><option value="completed">Concluída</option></select></label></div><div class="idea-map-actions"><div class="idea-zoom-controls" aria-label="Controles de zoom"><button type="button" id="ideaMapZoomOut" aria-label="Diminuir zoom" title="Diminuir zoom">−</button><button type="button" id="ideaMapZoomIn" aria-label="Aumentar zoom" title="Aumentar zoom">＋</button></div><button type="button" id="ideaMapCenter" title="Voltar ao centro">◎ <span>Centralizar</span></button><button type="button" id="ideaMapArrange" class="idea-arrange-button" title="Reorganizar as estrelas">✦ <span>Reorganizar</span></button></div></div></div><div class="idea-map-layout"><div class="idea-canvas-wrap"><canvas id="ideaMapCanvas" aria-label="Mapa visual de ideias"></canvas><div class="idea-map-help">Arraste estrelas · Arraste o fundo · Use zoom para explorar</div></div><aside class="idea-detail" id="ideaMapDetail"></aside></div></article>`;
