(() => {
  ...
  function layout(theme = mapState().theme) {
    ...
    all.forEach((note, index) => {
      ...
      else { const groupAngle = groupIndex / Math.max(1, groupEntries.length) * Math.PI * 2, centerRadius = groupEntries.length > 1 ? 175 : 0; x = Math.cos(groupAngle) * centerRadius + Math.cos(angle) * (45 + memberIndex * 11); y = Math.sin(groupAngle) * centerRadius + Math.sin(angle) * (45 + memberIndex * 11); }
  ...
  function build() {
    ...
    categorySelect.onchange = () => { mapState().selectedCategory = categorySelect.value; layout(); persist(); };
  ...
  window.LuarIdeaMap = { render, arrange: layout };
