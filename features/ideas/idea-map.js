(() => {
  ...
  function drawCategoryCenters(shownNotes) {
    ...
    grouped.forEach((items, key) => {
      ...
      ctx.save(); ctx.setLineDash([4, 6]); ctx.strokeStyle = `${info.color}55`; ctx.fillStyle = `${info.color}12`; ctx.lineWidth = selected ? 2 : 1; ctx.beginPath(); ctx.arc(center.x, center.y, radius, 0, Math.PI * 2); ctx.fill(); ctx.stroke(); ctx.setLineDash([]); ctx.fillStyle = selected ? "#fff" : info.color; ctx.font = `${selected ? 800 : 700} ${selected ? 11 : 9}px Inter, sans-serif`; ctx.textAlign = "center"; ctx.fillText(`${info.icon || "✦"} ${info.name}`, center.x, center.y - radius - 8); ctx.restore();
    });
  }

  function draw() {
    cancelAnimationFrame(raf); raf = requestAnimationFrame(() => {
      if (!canvas || !canvas.isConnected) return; const ratio = devicePixelRatio || 1, rect = canvas.getBoundingClientRect();
      ...
      ctx.setTransform(ratio, 0, 0, ratio, 0, 0); ctx.clearRect(0, 0, rect.width, rect.height); ctx.fillStyle = "#070b08"; ctx.fillRect(0, 0, rect.width, rect.height); ctx.save(); ctx.translate((camera.x * camera.zoom) + rect.width / 2, (camera.y * camera.zoom) + rect.height / 2); ctx.scale(camera.zoom, camera.zoom); drawBackdrop(mapState().theme, 0, 0);
      drawCategoryCenters(visibleNotes());
      const shown = new Set(visibleNotes().map(note => note.id));
      mapState().connections.forEach(link => { if (!shown.has(link.sourceId) || !shown.has(link.targetId)) return; const a = pos(notes().find(note => note.id === link.sourceId), false), b = pos(notes().find(note => note.id === link.targetId), false); if (!a || !b) return; const emphasized = hoverId && (link.sourceId === hoverId || link.targetId === hoverId); ctx.strokeStyle = emphasized ? "rgba(50,255,126,.72)" : "rgba(150,175,160,.22)"; ctx.lineWidth = emphasized ? 2 : 1; ctx.beginPath(); ctx.moveTo(a.x, a.y); ctx.lineTo(b.x, b.y); ctx.stroke(); });
      visibleNotes().forEach(note => { const point = pos(note), info = categoryInfo(note), radius = sizeOf(note), active = selectedId === note.id || hoverId === note.id; ctx.save(); ctx.translate(point.x, point.y); ctx.shadowColor = info.color; ctx.shadowBlur = note.glow === "strong" ? 28 : note.glow === "soft" ? 8 : 16; ctx.fillStyle = note.status === "completed" ? "#718078" : info.color; ctx.globalAlpha = note.status === "paused" ? .55 : 1; ctx.beginPath(); for (let i = 0; i < 10; i++) { const angle = -Math.PI / 2 + i * Math.PI / 5, r = i % 2 ? radius * .42 : radius; const x = Math.cos(angle) * r, y = Math.sin(angle) * r; i ? ctx.lineTo(x, y) : ctx.moveTo(x, y); } ctx.closePath(); ctx.fill(); if (active) { ctx.shadowBlur = 0; ctx.strokeStyle = "#fff"; ctx.lineWidth = 1.5; ctx.stroke(); } ctx.restore(); ctx.fillStyle = active ? "#fff" : "#aab6ae"; ctx.font = `${active ? 700 : 600} 10px Inter, sans-serif`; ctx.textAlign = "center"; ctx.fillText(String(note.title || "Ideia").slice(0, 28), point.x, point.y + radius + 14); });
      ctx.restore();
    });
  }
  ...
  function bindCanvas() {
    canvas.onpointerdown = event => { canvas.setPointerCapture(event.pointerId); const point = pointer(event), note = hit(point); if (note) { selectedId = note.id; dragging = { id: note.id, start: world(point) }; renderDetail(); draw(); } else panning = { point, x: camera.x, y: camera.y }; };
  ...
  function build() {
    ...
    searchInput.oninput = draw; prioritySelect.onchange = draw; statusSelect.onchange = draw; root.querySelector("#ideaMapCenter").onclick = () => { camera = { x: 0, y: 0, zoom: 1 }; draw(); }; root.querySelector("#ideaMapArrange").onclick = () => { Object.values(mapState().positions[mapState().theme]).forEach(point => point.locked = false); layout(); persist(); };
  ...
  function renderDashboardMiniMap() {
    const host = document.getElementById("dashboardWidgetZone"); if (!host) return; host.querySelector(".dashboard-idea-map-widget")?.remove();
  ...
  window.LuarIdeaMap = { render, arrange: layout, renderDashboard: renderDashboardMiniMap };
