(() => {
  ...
  function renderSettings() {
    ...
    const chosenCharacter = typeof profileAvatarUrl === "function" ? profileAvatarUrl() : "";
    page.innerHTML = `<div class="page-head"><div><span>PERSONALIZAÇÃO</span><h2>Widgets da Visão Geral</h2><p>Fixe tarefas e hábitos para acompanhar e concluir sem sair do painel.</p></div><button class="secondary" type="button" data-page="dashboard">Ver Visão Geral</button></div><section class="widget-config-hero">${chosenCharacter ? `<img class="widget-config-user-character" src="${safe(chosenCharacter)}" alt="Personagem escolhido pelo usuário">` : ""}<div><span>SUA ÓRBITA EM DESTAQUE</span><h3>O que merece ficar sempre à vista?</h3><p>${chosenCharacter ? "Seu personagem escolhido acompanha" : "Sua órbita acompanha"} tarefas e hábitos usando os registros originais da conta.</p><strong>${widgets().length} / ${limit()} <small>widgets utilizados</small></strong></div></section><div class="widget-config-layout"><section class="card widget-picker"><header><span>ADICIONAR WIDGET</span><h3>Escolha o tipo e o registro</h3></header><div class="widget-type-tabs"><button type="button" class="active" data-widget-tab="task">✓ Tarefas</button><button type="button" data-widget-tab="habit">✦ Hábitos</button></div><div class="widget-record-options" data-widget-options="task"></div><div class="widget-record-options" data-widget-options="habit" hidden></div></section><section class="card widget-current"><header><span>ORDEM ATUAL</span><h3>Widgets fixados</h3></header><div id="widgetSettingsCurrent"></div></section></div>`;
