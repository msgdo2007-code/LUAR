(() => {
  ...
  function bindCards(root) {
    root.querySelectorAll("[data-widget-id]").forEach(card => { const widget = widgets().find(item => item.id === card.dataset.widgetId); if (!widget) return; const reveal = () => { const open = card.classList.toggle("is-open"); card.setAttribute("aria-expanded", String(open)); }; card.onclick = event => { if (!event.target.closest("button,nav")) reveal(); }; card.onkeydown = event => { if ((event.key === "Enter" || event.key === " ") && event.target === card) { event.preventDefault(); reveal(); } }; card.querySelector(".widget-complete")?.addEventListener("click", () => toggle(widget)); card.querySelector("[data-widget-remove]")?.addEventListener("click", () => remove(widget.id)); card.querySelectorAll("[data-widget-move]").forEach(button => button.addEventListener("click", () => move(widget.id, Number(button.dataset.widgetMove)))); card.querySelector("[data-widget-config]")?.addEventListener("click", () => showPage("widgets")); });
  }
