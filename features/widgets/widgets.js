(() => {
  ...
  function renderCard(widget, index) {
    ...
    const streak = widget.type === "habit" ? calculateHabitStreak(item) : 0;
    card.innerHTML = `<header><i>${icon(widget.type)}</i><span><small>${label(widget.type).toUpperCase()} FIXADO</small><b></b></span><button type="button" data-widget-config aria-label="Configurar widget">•••</button></header><p></p><footer><button type="button" class="widget-complete">${complete(widget, item) ? "✓ Concluído hoje" : widget.type === "task" ? "Concluir tarefa" : "Marcar como feito"}</button><span>${widget.type === "habit" ? `${streak} dia${streak === 1 ? "" : "s"} de sequência` : item.date ? `Prazo ${formatDate(item.date)}` : "Sem prazo"}</span></footer><nav aria-label="Ordenar widget"><button type="button" data-widget-move="-1" ${index === 0 ? "disabled" : ""}>←</button><button type="button" data-widget-move="1" ${index === widgets().length - 1 ? "disabled" : ""}>→</button><button type="button" data-widget-remove>Remover</button></nav>`;
    card.querySelector("header b").textContent = item.name;
    card.querySelector("p").textContent = item.description || item.category || (widget.type === "task" ? "Seu próximo passo importante." : "Continue construindo sua constância.");
    return card;
  }
