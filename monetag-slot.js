...
const observeAdResponse=(url,status)=>{if(!/\/11547574(?:\?|$)/.test(String(url||'')))return;if(status===204)markReady('unavailable');else if(status>=200&&status<400)markReady('ready')};
...
(function(s){s.dataset.zone='11547574',s.src='https://n6wxm.com/vignette.min.js'})([document.documentElement, document.body].filter(Boolean).pop().appendChild(document.createElement('script')));
const monetagScript=document.querySelector('script[data-zone="11547574"]');
