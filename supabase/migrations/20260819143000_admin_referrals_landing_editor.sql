...
create table if not exists public.landing_page_versions (
  ...
  revision integer not null, content jsonb not null check(jsonb_typeof(content)='object'), summary text not null check(char_length(summary) between 3 and 200),
...
begin
 ...
 select coalesce(jsonb_agg(to_jsonb(v) order by v.created_at desc),'[]'::jsonb) into v_versions from (select id,revision,summary,created_at from public.landing_page_versions where slug='home' order by revision desc limit 20) v;
...
begin
 ...
 insert into public.landing_page_versions(slug,revision,content,summary,published_by) values('home',v_doc.published_revision,v_doc.published,v_summary,v_actor);
