...
module.exports = async (req, res) => {
  ...
  try {
    ...
    if (lifetime) account = await upsertLuarAccount({ email, user_ids: [...new Set([...(account?.user_ids || []), user.id])], plan: "lifetime", lifetime_paid_at: account?.lifetime_paid_at || license.paidAt || new Date().toISOString(), lifetime_transaction_id: account?.lifetime_transaction_id || license.transactionId || null, updated_at: new Date().toISOString() });
